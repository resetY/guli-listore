package com.yuan.guli.gulimember.utils;
...
public class MD5Util {
  ...
  /**
   * 自带md5加密工具方法：进行盐值拼串 + 大写化 + 三次md5加密
   * @param password 前端传输过来的密码
   * @param  salt 业务层随机传输过来的盐值
   * @return  password 最终加密后的密码
   * **/
   public final static String getMd5_Three(String password,String salt){
        for(int i = 0;i<3;i++){
    ...
    /**
     * 米米商城md5加密工具方法：进行盐值拼串 + 大写化 + 三次md5加密
     * @param password 前端传输过来的密码
     * @param  salt 业务层随机传输过来的盐值
     * @return  password 最终加密后的密码
     * **/
   public  final static String getMd5_Three2(String password,String salt){
        for(int i = 0;i<=3;i++){
          password =  MD5Util.getMD5((salt+password+salt)).toUpperCase();
        }
        return password;
    }
